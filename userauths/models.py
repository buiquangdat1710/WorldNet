...
class User(AbstractUser):
    ...
    is_online = models.BooleanField(default=False)  # Thêm trường này
    last_active = models.DateTimeField(auto_now=True)  # Thêm trường này
    ...
    def is_active_now(self):
        """
        Kiểm tra xem user có đang active trong 5 phút gần đây không
        """
        now = timezone.now()
        return self.is_online and (now - self.last_active) < timedelta(minutes=5)
